...
import { ROLE_IDS, PERMISSION_IDS, SCOPE_LEVELS } from './pr35-contracts.js';
...
export function createOwnerController({ root = document, repository, careAdapter, identity, clock = () => new Date().toISOString(), local = false }) {
  ...
  function openAssignment(subjectId = 'profile-user') {
    ...
    next.addEventListener('click', () => {
      ...
      const confirm = make('button', { type: 'button', class: 'pr35-primary', 'data-assignment-confirm': '' }, local ? 'تأكيد التكليف المحلي' : 'تأكيد التكليف الآمن'); form.append(confirm); confirm.focus();
