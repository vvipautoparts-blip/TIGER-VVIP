...
scan_pattern() {
  ...
  while IFS=: read -r file line _rest; do
    ...
    --exclude-dir=.venv \
    --exclude=p08-steel-shield-secret-scan.test.cjs \
    -- "$pattern" "$TARGET_DIR" || true)
}

scan_pattern "SUPABASE_SERVICE_ROLE" '(SUPABASE_SERVICE_ROLE_KEY|service_role)[[:space:]]*[:=][[:space:]]*["'"'"'][A-Za-z0-9._-]{20,}["'"'"']'
scan_pattern "SUPABASE_ACCESS_TOKEN" 'sbp_[A-Za-z0-9_-]{20,}'
scan_pattern "CLERK_SECRET_KEY" 'sk_(live|test)_[A-Za-z0-9]{20,}'
...
scan_pattern "POSTGRES_URL" 'postgres(ql)?://[^[:space:]/:@]+:[^[:space:]@]+@[^[:space:]]+'
