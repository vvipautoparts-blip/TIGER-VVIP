(function (root) {
  ...
  function renderFilters() {
    const host = root.document.querySelector("[data-nexus-sector-filters]");
    if (!host) return;
    const available = new Set(state.items.map((item) => item.sectorKey));
    const controls = [
      '<button class="filter is-active" type="button" data-nexus-sector-filter="all" aria-pressed="true">الكل</button>'
    ];
    for (const entry of registry()) {
      if (!available.has(entry.key)) continue;
      controls.push(`<button class="filter" type="button" data-nexus-sector-filter="${safeText(entry.key)}" aria-pressed="false">${safeText(entry.label)}</button>`);
    }
  ...
  function render() {
    const feed = root.document.querySelector("[data-nexus-sector-discovery-feed]");
    const count = root.document.querySelector("[data-nexus-sector-results-count]");
    const empty = root.document.querySelector("[data-nexus-sector-empty]");
  ...
  function bindControls() {
    const search = root.document.querySelector("[data-nexus-sector-search]");
    if (search) {
    ...
    root.document.addEventListener("click", function (event) {
      const filter = event.target && event.target.closest
        ? event.target.closest("[data-nexus-sector-filter]")
        : null;
      if (!filter) return;
      const key = filter.getAttribute("data-nexus-sector-filter");
      if (key !== "all" && !registry().some((entry) => entry.key === key)) return;
      state.sector = key || "all";
      for (const button of root.document.querySelectorAll("[data-nexus-sector-filter]")) {
