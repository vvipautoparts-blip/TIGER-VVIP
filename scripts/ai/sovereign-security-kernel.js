...
// AI-02 deliberately models trusted server issuance in-memory. Persistent,
// cross-process cryptographic proof is introduced by the AI-03 trust fabric.
...
function assertCanonicalValue(value, seen) {
  ...
  if (type !== 'object') {
    throw new TypeError(`Unsupported ${type} value in payload.`);
  }

  if (seen.has(value)) {
    throw new TypeError('Unsupported cyclic value in payload.');
  }
  seen.add(value);
...
function verifyApprovalEnvelope({ approval, actor, agentId, action, payload, now, consumedIds } = {}) {
  ...
  if (!actor?.authenticated || actor.role !== 'OWNER') {
    return Object.freeze({ ok: false, reasonCode: 'TRUSTED_OWNER_REQUIRED' });
  }
  if (String(actor.id || '') !== approval.ownerId) {
    return Object.freeze({ ok: false, reasonCode: 'OWNER_ID_MISMATCH' });
  }
  ...
  if (current > expires) {
    return Object.freeze({ ok: false, reasonCode: 'APPROVAL_EXPIRED' });
  }
...
function applyBudgetGate({ spent, requested, limit } = {}) {
  ...
  if (spent + requested > limit) {
    return Object.freeze({ ok: false, reasonCode: 'BUDGET_EXCEEDED' });
  }
...
function applyRateGate({ used, requested, limit } = {}) {
  ...
  if (used + requested > limit) {
    return Object.freeze({ ok: false, reasonCode: 'RATE_LIMIT_EXCEEDED' });
  }
...
function evaluateTool(tool, agentId) {
  ...
  if (!definition.allowedAgents.includes(agentId)) {
    return deny('TOOL_SCOPE_DENIED', { toolId: definition.id });
  }
  return null;
}
...
function evaluateSovereignRequest({
  ...
  if (!actor?.authenticated) return deny('AUTHENTICATION_REQUIRED', { action, level: rule.level });
  if (!featureEnabled) return deny('FEATURE_DISABLED', { action, level: rule.level });
  if (killSwitches.global || killSwitches.agent || killSwitches.tool) {
  ...
  if (budget) {
    const budgetDecision = applyBudgetGate(budget);
    if (!budgetDecision.ok) return deny(budgetDecision.reasonCode, { action, level: rule.level });
  }
  if (rate) {
    const rateDecision = applyRateGate(rate);
    if (!rateDecision.ok) return deny(rateDecision.reasonCode, { action, level: rule.level });
  }
  ...
  if (rule.decision === DECISIONS.OWNER_APPROVAL_REQUIRED) {
    ...
    return allow('TRUSTED_OWNER_APPROVAL', {
      action,
      agentId,
      level: rule.level,
