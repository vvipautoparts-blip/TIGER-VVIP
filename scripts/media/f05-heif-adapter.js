(function(root,factory){
  ...
  const WORKER_POLICY=Object.freeze({maxPixels:40000000,minWidth:320,minHeight:240,maxWidth:1600,maxHeight:1200,webpQuality:0.82,jpegQuality:0.86});
  ...
  function validWorkerJob(job){
    if(!validJob(job)||!job.transform||![job.transform.zoom,job.transform.panX,job.transform.panY].every(finite)||!job.policy)return false;
    return Object.keys(WORKER_POLICY).every(key=>job.policy[key]===WORKER_POLICY[key]);
  }
  ...
  function buildWorkerTransfer(job){
    if(!validWorkerJob(job))throw error('heif_container_invalid');
    const transform=Object.freeze({zoom:job.transform.zoom,panX:job.transform.panX,panY:job.transform.panY});
    const policy=Object.freeze(Object.keys(WORKER_POLICY).reduce((copy,key)=>{copy[key]=job.policy[key];return copy;},{}));
    const workerJob=Object.freeze({jobId:job.jobId,bytes:job.bytes,mimeType:job.mimeType,transform,policy});
    const message=Object.freeze({type:'process',job:workerJob});
    return Object.freeze({message,transfer:Object.freeze([job.bytes])});
  }
