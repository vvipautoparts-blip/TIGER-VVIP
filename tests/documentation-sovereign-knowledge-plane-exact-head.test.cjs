...
const workflow = fs.readFileSync(
  path.join(ROOT, ".github/workflows/documentation-sovereign-knowledge-plane.yml"),
  "utf8"
);

test("Documentation Sovereign Knowledge Plane checks out and proves the exact PR head SHA", () => {
  assert.ok(
  ...
  assert.match(workflow, /Verify repository remains clean/);
});
