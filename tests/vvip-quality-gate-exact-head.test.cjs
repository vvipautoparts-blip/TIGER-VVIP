...
test("VVIP Quality Gate checks out and proves the exact PR head SHA", () => {
  assert.ok(
  ...
  assert.ok(
    workflow.includes("name: vvip-quality-gate-${{ env.SOURCE_SHA }}"),
    "diagnostic artifact must be named with the verified source SHA"
  );
});
