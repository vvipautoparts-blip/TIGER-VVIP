...
test('release, artifact, environment, workload, statement and registry mismatches fail closed', () => {
  ...
  expectCode(() => createIdentityTransparencyConstellation({ expectedReleaseDnaSha256: HEX('0'), expectedEnvironment: 'staging', expectedArtifactSha256: ARTIFACT, expectedWorkloadRefSha256: WORKLOAD, expectedStatementSha256: STATEMENT, expectedRegistryRefSha256: REGISTRY, clock: () => NOW }).derive({ bridgeResult: bridge, workloadIdentity: workload, transparencyResult: transparency }), 'TRUST_CONSTELLATION_RUNTIME_MISMATCH');
