...
test("repost persistence preserves original identity, privacy intersection, and one active repost per actor", () => {
  const sql = fs.readFileSync(repostMigrationPath, "utf8");

  assert.match(sql, /create table public\.vvip_social_reposts/i);
  assert.match(sql, /original_post_id uuid not null/i);
  assert.doesNotMatch(sql, /original_post_id uuid not null references public\.vvip_social_posts/i);
  ...
  assert.match(sql, /repost\.repost_post_id = post\.post_id/i);
  assert.match(sql, /original\.post_id = repost\.original_post_id/i);
  assert.match(sql, /SOCIAL_REPOST_CHAIN_FORBIDDEN/i);
  ...
  assert.match(sql, /create trigger vvip_social_repost_snapshot_sync/i);
