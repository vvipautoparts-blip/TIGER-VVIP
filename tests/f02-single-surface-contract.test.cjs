...
const jsPath = path.join(root, 'scripts/fusion/f02-single-surface.js');
...
test('F02 Single Surface assets replace legacy marketplace presentation path', () => {
  ...
  assert.equal(fs.existsSync(jsPath), true, 'F02 Single Surface controller must exist');

  const html = fs.readFileSync(indexPath, 'utf8');
  assert.match(html, /styles\/fusion\/f02-single-surface\.css/);
  assert.match(html, /scripts\/fusion\/f02-single-surface\.js/);
...
test('F02 controller is synthetic-preview-only by default and exposes exactly three primary card actions', () => {
