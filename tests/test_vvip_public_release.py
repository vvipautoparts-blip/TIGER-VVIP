...
class PublicReleaseTests(unittest.TestCase):
    def fixture(self, root: Path, *, test_key: bool = False, fixture_data: bool = False) -> None:
        ...
        (root / "scripts" / "runtime" / "vvip-marketplace-rollback.js").write_text("legacy-wrapper", encoding="utf-8")
        (root / "scripts" / "runtime" / "vvip-my-listings.js").write_text("legacy-runtime", encoding="utf-8")
        (root / "styles").mkdir()
    ...
    def test_candidate_excludes_internal_paths_and_transforms_index(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertNotIn("vvip-marketplace-rollback.js", index)
            self.assertIn('href="#marketplace"', index)
            self.assertIn("data-account-route", index)
            self.assertIn("runtime-config.js", index)
            self.assertIn("vvip-runtime-loader.js", index)
            self.assertIn("vvip-marketplace-repository.js", index)
            self.assertTrue((output / "scripts" / "runtime" / "vvip-runtime-loader.js").is_file())
            self.assertTrue((output / "scripts" / "runtime" / "vvip-marketplace-repository.js").is_file())
            self.assertFalse((output / "scripts" / "runtime" / "vvip-marketplace-rollback.js").exists())
            self.assertFalse((output / "scripts" / "runtime" / "vvip-my-listings.js").exists())
    ...
    def test_production_build_succeeds_with_clean_sources(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("scripts/runtime/vvip-runtime-loader.js", manifest["files"])
            self.assertIn("scripts/runtime/vvip-marketplace-repository.js", manifest["files"])
            self.assertNotIn("scripts/runtime/vvip-marketplace-rollback.js", manifest["files"])
            self.assertNotIn("scripts/runtime/vvip-my-listings.js", manifest["files"])
