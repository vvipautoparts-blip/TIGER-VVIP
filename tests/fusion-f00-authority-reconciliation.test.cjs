...
test('F00 current authority files exist and point to FINAL FUSION constitution', () => {
  for (const file of ACTIVE_REFERENCE_FILES) {
    ...
    assert.match(text, new RegExp(FINAL_CONSTITUTION.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')),
      `${file} must point to FINAL FUSION constitution`);
  }

  const ownerReference = read('docs/VVIP_TIGER_CURRENT_OWNER_REFERENCE.md');
  assert.match(ownerReference, new RegExp(MASTER_ROADMAP.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')),
    'current owner reference must point to F00-F16 master roadmap');
});

test('F00 machine-readable decision catalog is canonical and fully supersedes legacy authority', () => {
  ...
  for (const id of LEGACY_IDS) {
    ...
    assert.equal(decision.reason, 'Owner-approved FUSION 2026 constitution');
  }
});

test('F00 active authority references contain no superseded implementation instructions', () => {
  for (const file of ACTIVE_REFERENCE_FILES) {
    const text = read(file);
...
test('F00 old authority paths are tombstones, not alternate product constitutions', () => {
  for (const file of [
    'docs/VVIP_TIGER_OFFICIAL_PRODUCT_BLUEPRINT.md',
    'docs/VVIP_TIGER_MEMORY_MAP.md',
    'IMPLEMENTATION_CHECKLIST.md'
