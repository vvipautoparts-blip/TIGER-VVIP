...
test("canonical repository finalizes the inserted media rows without a redundant database re-read", () => {
  assert.match(source, /mediaRows\s*=\s*await\s+uploadMedia\s*\(/);
  assert.match(source, /for\s*\(const\s+media\s+of\s+mediaRows\)/);
  assert.match(source, /await\s+finalizeMediaRow\s*\(/);
  assert.doesNotMatch(source, /MEDIA_FINALIZATION_ROWS_READ_FAILED/);
