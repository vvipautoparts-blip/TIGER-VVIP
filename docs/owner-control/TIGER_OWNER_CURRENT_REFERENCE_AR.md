...
- تجربة المنتج الحالية: `TIGER NEXUS 2026`.
- TIGER شبكة اجتماعية عالمية متخصصة بالقطاعات المفعلة فقط؛ لا يوجد نشر عام جديد بلا قطاع وغرض معتمد.
...
- `☰` هو `TIGER Command` الدائم، و`صلاحياتي` Capability Passport بشري مشتق فقط من حالة صلاحيات مؤكدة من الخادم؛ الحالة null/unvalidated لا تعرض أي إجراء مميز.
...
- الظهور المدفوع الحالي: `TIGER PULSE RING / PULSE VAULT` بأسعار خدمة منصة أساسية **2 / 10 / 20 / 45 JOD** فقط.
- هذه الأسعار الأساسية لا تحتوي خط أساس ضريبي 16% ولا سقف ضريبة 16%.
- الضريبة القانونية الموثقة حسب الدولة/المعاملة تُضاف فوق سعر خدمة المنصة الأساسي: `السعر النهائي = السعر الأساسي + الضريبة القانونية المطبقة`.
- الضريبة القانونية تبقى خارج نسب التوزيع والعمولات، ولا تحدد الدولة المفتوحة أو المغلقة بحد ذاتها.
- كل مبلغ أساسي له تخصيص ظهور server-authoritative مختلف ويظهر للمستخدم السعر الأساسي والضريبة الموثقة والسعر النهائي قبل الدفع؛ الظهور لا يقاس بالأيام ولا ينتهي بمرور الزمن أو بسبب الانتظار قبل الاستخدام.
...
- إذا لم يوجد GENERAL_MANAGER أو SECTOR_MANAGER أو MARKETER منسوب للبيع، يحصل المستخدم على خصم خدمة ذاتية **7%** على خدمة المنصة قبل إغلاق العرض المالي الخاضع للمعالجة الضريبية القانونية الموثقة.
...
تم إلغاء `TAX_RESERVE 16%` الداخلي من التوزيع الحالي. هذا البند الملغى **ليس الضريبة القانونية** وليس خط أساس للتسعير. لا تُرحّل هذه الـ16% تلقائيًا إلى المالك أو التشغيل أو الشركاء أو المبيعات أو CSR أو أي بند آخر.
...
- النسبة الداخلية المتبقية بعد إلغاء TAX_RESERVE: **16% بانتظار قرار مالك صريح لإعادة توزيعها**
...
`7% GENERAL_MANAGER + 7% SECTOR_MANAGER + 7% MARKETER`، لكن في كل عملية شراء **فائز بشري واحد فقط** يأخذ 7%، والباقي يرحل للمالك مع reason codes واضحة.

عند الشراء الذاتي بلا بائع: يطبق خصم المستخدم 7% على خدمة المنصة أولًا وفق السلطة الحالية، ثم تُحسب الضريبة القانونية الموثقة على الأساس القانوني المطبق، ولا عمولة لأي دور مبيعات، و21% من **أساس توزيع إيراد خدمة المنصة المستبعد منه الضريبة القانونية** ترحل للمالك مع توثيق غياب الأدوار.

كل دور `DIGITAL` يبقى صفر منفعة مالية: لا عمولة، لا حصة، لا استحقاق، لا payout destination، لا wallet، ولا ملكية لعمولة بيع.

التسوية للعمولات البشرية المؤهلة كل 14 يومًا، ووجهة الصرف مطلوبة خلال 12 ساعة من منح الدور إلا إذا مدد المالك المهلة. تصفير الرصيد بعد التسوية لا يعني حذف سجل الحركات؛ الـledger يبقى غير قابل للمحو.

إلى أن يقرر المالك توزيع الـ16% الداخلية الملغاة، تنفيذ التوزيع المالي النهائي يبقى `FAIL_CLOSED` ولا يتم اختراع مستفيد بديل.
...
4. `TIGER_PULSE_RING_2026_CURRENT_OWNER_AUTHORITY.md` — أسعار Pulse الأساسية 2/10/20/45 والظهور بلا مدة.
5. `TIGER_FINANCIAL_DISTRIBUTION_CURRENT.md` — السلطة المالية الحالية مع إلغاء TAX_RESERVE وعدم اختراع توزيع بديل للـ16%.
6. `TIGER_STATUTORY_TAX_BOUNDARY_CURRENT.md` — فصل الضريبة القانونية عن اقتصاد التوزيع وإضافتها فوق سعر خدمة المنصة كما ينطبق قانونيًا.
7. `TIGER_PHOENIX_CLEANROOM_2026_CURRENT_OWNER_AUTHORITY.md` — حوكمة التنظيف.
8. `TIGER_AION_2026_CURRENT_OWNER_AUTHORITY.md` — سلطة `post-launch-autonomy` المستقبلية المحمية.
9. `project-control/authority/authority-registry.v1.json` — Authority Graph الآلي.
10. `config/fusion/current-authority.json` — عقد المنتج الآلي.
11. `config/finance/current-distribution.json` — عقد التوزيع المالي الآلي.
12. `config/launch/global-launch-passport.json` — حقيقة أهلية الإطلاق العالمي الآلية.
13. exact Git SHA/tree + matching CI evidence — حقيقة التنفيذ.
