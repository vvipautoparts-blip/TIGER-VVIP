...
| Home Feed | Social posts as Home authority | `PARTIAL` | Social Home surface exists; legacy marketplace no longer intended as whole-product Home | Bind trusted social-post read model |
| Post Composer | Text/image post separate from commercial listing | `PARTIAL` | Social composer shell + fail-closed PostIntent domain exist | Bind server publication adapter and audience state |
| Social Posts DB | Owner-bound posts with public/friends/only_me visibility | `PARTIAL` | TDD migration under PR #271; not applied to database | Security review, rehearsal, staged apply gate |
| Social Graph / Friends | send, cancel, receive, accept, decline, unfriend | `PARTIAL` | Fail-closed relationship state machine + TDD DB foundation under PR #271 | Bind server persistence and UI states |
...
| Reactions | Like/reaction state and counts | `NOT_STARTED` | Legacy feed counters are not trusted Social authority | Domain contract + RLS model |
| Comments | comments, replies, edit/delete, counts | `NOT_STARTED` | No accepted current Social Core implementation | Domain contract + moderation/privacy model |
