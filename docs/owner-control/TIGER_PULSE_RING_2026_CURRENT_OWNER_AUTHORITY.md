...
A sovereign price exists only through an explicitly authorized **Market Pricing Contract** that binds at least the relevant market, product level, amount, currency, policy version, and applicable validity/evidence.
...
Each Pulse product level maps through the server-authoritative visibility engine to its defined visibility allocation under the applicable market contract. Before payment the user must see the actual market-authorized amount/currency and resulting visibility information, including the exact server quote available for that scope.
...
Technical/security expirations such as short-lived payment quotes, OTPs, sessions, signed URLs, replay windows, caches, owner execution leases, and execution seals are allowed and are not product-duration rules.
...
Targeting must be server-validated against current sector/geographic registries and the active sovereign Market Genome/contract. Payment cannot bypass safety, policy, identity, market activation, relevance, quality, trust, inventory, privacy, or legal eligibility.
...
Payment requires a Market Pricing Contract, server-authoritative quote verification, an authorized market payment profile, provider-authoritative result, idempotency, replay protection, and immutable accounting/audit evidence.
...
If any required market, price, currency, payment profile, legal/tax policy, or SGF capability authority is missing/invalid/expired/revoked:
...
- infer market from IP, locale, hosting region, or provider location;
- activate another payment provider unless the same market policy explicitly authorizes that failover.

## 12. Superseded material disposal

Conflicting publishing cards, subscriptions, paid publishing slots, timed activation cards, global JOD price authority, product-duration fields, and paid-publication entitlement gates must be removed from the current repository tree and active schema/runtime through protected forward migration where necessary.
