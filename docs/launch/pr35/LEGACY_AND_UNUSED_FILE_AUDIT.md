...
| Historical source snapshots | removed from the active tree after canonical runtime proof | recover only from Git history when needed |
| root and migration SQL | historical/production-sensitive | no change; PR35 SQL only under review path |
| `.vscode/launch.json` | stale public-profile launch URLs were present during PR35 | repaired later by clean-room stabilization |
...
| Duplicate profile snapshots | confirmed absent from runtime imports and removed during clean-room stabilization | canonical files remain in the active tree |
