...
The application stores only an allowlisted in-memory intent name and bounded payload needed to resume the action. No access token, OAuth response, password, or sensitive authentication material is placed in localStorage/sessionStorage.
...
On successful sign-in, the orchestrator consumes and clears the intent before executing its callback. This gives at-most-once resume behavior and prevents repeated side effects if Clerk emits multiple listener updates.

## Authentication UI strategy

Use Clerk's maintained prebuilt `<SignIn />` component rather than a hand-written OAuth implementation in P0. The current Clerk JavaScript SDK supports hash routing and prebuilt social/passkey strategies based on production instance settings. This choice minimizes protocol and callback risk while still allowing VVIP visual framing around the auth experience.
...
- Authentication intent payloads are bounded and in-memory only.
- Public browsing does not imply anonymous account creation.

## Files and responsibilities

- `auth-clerk-index.js`: guest-first auth orchestrator, access-sheet lifecycle, bounded intent/resume behavior, safe Clerk mounting and recovery.
- `scripts/vvip-production-marketplace.js`: public read behavior and calls into the orchestrator for protected actions.
...
8. No VVIP-owned password input or password storage is introduced.
9. No auth token/provider error payload is logged.
10. Existing RLS/authenticated mutation controls remain unchanged.
11. Focused auth/marketplace tests and the full repository quality gate pass on one head SHA.
12. Production is not deployed from this branch until protected review/approval gates are satisfied.
