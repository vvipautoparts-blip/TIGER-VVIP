...
8. Web/PWA مستمر كقناة أساسية.
9. Android وiPhone مطلوبان Store-ready ضمن هدف إطلاق 60 يومًا.
10. التنفيذ المستهدف للتطبيقات Cross-platform/Hybrid.
11. لا يجوز تأجيل Android أو iPhone إلى مرحلة غير محددة بعد الإطلاق.
...
5. مدة ظهور الإعلان مرتبطة بالمدة المتبقية من اشتراك المستخدم.
6. يُرسل تنبيه قبل انتهاء الاشتراك بخمسة أيام.
7. عند انتهاء الاشتراك يتحول الحساب إلى خمول اشتراك.
8. عند خمول الاشتراك تُخفى الإعلانات وتتوقف مزايا البطاقة.
9. عند بلوغ الإعلان 40 يومًا من تاريخ نشره يُحذف الإعلان وصوره المعالجة تلقائيًا.
10. لا تعتمد صياغة «الأقصر بين المدة المتبقية من الاشتراك و40 يومًا» كقاعدة مستقلة.
11. الارتباط بالاشتراك يحكم الظهور، بينما 40 يومًا هو حد الحذف النهائي.

### خمول الحساب وخمول الاشتراك (القرار الأحدث المعتمد)

1. يوم 30: إشعار.
2. يوم 40: تعطيل وإخفاء.
3. مراحل ما بعد يوم 40 أو الحذف النهائي لا تُعتمد إلا إذا ثبتت صراحة في المرجع الأحدث المعتمد.
4. عند وجود تعارض تاريخي بين صياغات خمول قديمة وصياغة نهائية أحدث، تُوسم الصياغة الأقدم على أنها Superseded.
...
## حفظ الميثاق وتعليمات المساعد داخل المنصة

1. الميثاق الأحدث هو المرجع الحالي المعتمد.
2. الميثاق محفوظ داخل المنصة ومرتبط بحساب المالك.
3. جميع أكواد وتعليمات المساعد الموجودة داخل المستند المرجعي محفوظة داخل المنصة.
4. يجوز الاستعانة بهذه الأكواد والتعليمات تلقائيًا لتجهيز VVIP TIGER ولمصلحة المنصة.
5. يشمل الإذن الداخلي عند الحاجة: تنفيذ الأوامر، تعديل الملفات، إنشاء الفروع، إنشاء Commits، فتح Draft Pull Requests، تشغيل الاختبارات والفحوصات، ومعالجة النواقص والتعارضات المثبتة.
6. الاستخدام يكون داخل المنصة ومستودعاتها المعتمدة فقط.
7. لا تُنقل أو تُشارك هذه المواد خارج المنصة إلا وفق الصلاحيات والسياسة المعتمدة.
8. أي قرار أو تعديل يعتمده المالك يُدخل مباشرة في الملف المرجعي وتُحفظ نسخة منقحة.
9. يمنع حفظ Secrets أو Tokens أو كلمات مرور أو Clerk Secret Keys أو Supabase service_role ضمن المستندات أو أكواد المساعد.
10. الإذن التلقائي لا يشمل: الدمج، الإنتاج، حذف البيانات، حذف الفروع، تغيير الفوترة، تدوير الأسرار، أو تعديل الأنظمة الخارجية الحساسة إلا بموافقة صريحة من المالك.
11. عند وجود تعارض أمني أو قانوني أو شرعي غير محسوم، يتوقف التنفيذ ولا يُعتمد التخمين.
...
1. هذا PR يحتوي ملف مواصفة Markdown واحدًا فقط.
2. لا HTML.
3. لا CSS.
4. لا JavaScript.
5. لا Runtime.
6. لا SQL.
7. لا Supabase migrations.
8. لا Tests.
9. لا Clerk أو Supabase configuration.
10. لا أسرار.
11. لا تغييرات إنتاجية.
...
6. أي صياغة تؤجل تطبيقات Android وiPhone إلى وقت غير محدد أصبحت Superseded.
7. صياغة «الأقصر بين مدة الاشتراك و40 يومًا» كقاعدة مستقلة أصبحت Superseded.
8. أي قاعدة قديمة لخمول الحساب تتعارض مع الآلية النهائية الأحدث (يوم 30 إشعار، يوم 40 تعطيل وإخفاء) تصبح Superseded.
9. PR #86 يصبح Superseded فقط بعد نجاح PR البديل النظيف وإثبات عزله من origin/main.
