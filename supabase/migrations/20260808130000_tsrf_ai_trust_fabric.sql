-- TSRF semantic convergence: sovereign AI trust fabric.
-- Non-production source migration. Browser roles have no direct authority.
...
create table if not exists public.ai_approval_requests (
  ...
  constraint ai_approval_environment_check check (
    environment in ('STAGING', 'REPOSITORY', 'PRODUCTION')
  ),
  constraint ai_approval_release_digest_check check (release_digest ~ '^[0-9a-f]{64}$'),
  constraint ai_approval_payload_digest_check check (payload_digest ~ '^[0-9a-f]{64}$'),
  ...
  constraint ai_approval_approved_time_check check ((status <> 'approved') or approved_at is not null),
  constraint ai_approval_consumed_time_check check ((status <> 'consumed') or consumed_at is not null)
);
...
create index if not exists ai_approval_requests_release_binding_idx
  on public.ai_approval_requests (release_digest, payload_digest, scope_digest, environment);
...
create table if not exists public.ai_audit_events (
  ...
  created_at timestamptz not null default now(),
  constraint ai_audit_decision_check check (decision in ('ALLOW', 'DENY', 'OWNER_APPROVAL_REQUIRED', 'ERROR')),
  constraint ai_audit_release_digest_check check (release_digest ~ '^[0-9a-f]{64}$'),
  ...
  constraint ai_audit_previous_hash_check check (previous_hash is null or previous_hash ~ '^[0-9a-f]{64}$'),
...
create table if not exists public.ai_usage_ledger (
  ...
  country_code text,
  created_at timestamptz not null default now(),
  constraint ai_usage_release_digest_check check (release_digest ~ '^[0-9a-f]{64}$'),
...
create table if not exists public.ai_prompt_versions (
  ...
  constraint ai_prompt_agent_check check (
    agent_id in ('general_manager', 'technical_manager', 'financial_analytics_manager', 'user_assistant')
  ),
...
revoke all on function public.consume_ai_owner_approval(uuid, text, text, text, text, text, text, text, timestamptz) from public, anon, authenticated;
grant execute on function public.consume_ai_owner_approval(uuid, text, text, text, text, text, text, text, timestamptz) to service_role;
