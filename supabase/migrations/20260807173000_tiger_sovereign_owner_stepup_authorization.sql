-- TIGER SOVEREIGN AI-18 owner step-up authorization persistence
-- Repository change only. Remote database application remains governed by
-- preview/staging review, protected DB promotion, and owner activation gates.
...
create table if not exists public.ai_owner_stepup_authorizations (
  ...
  constraint ai_owner_stepup_action_environment_check check (
    ...
    or (action = 'PROMOTE_DATABASE' and environment = 'PRODUCTION')
    or (action = 'ACTIVATE_PRODUCTION' and environment = 'PRODUCTION')
    or (action = 'CHANGE_PRICES' and environment = 'PRODUCTION')
    or (action = 'CHANGE_OWNER_SECURITY' and environment = 'PRODUCTION')
    or (action = 'CHANGE_AI_SECURITY_POLICY' and environment = 'PRODUCTION')
  ),
  constraint ai_owner_stepup_method_check check (
    auth_method in ('WEBAUTHN_PASSKEY', 'IDP_PHISHING_RESISTANT_MFA')
  ),
  constraint ai_owner_stepup_assurance_check check (
  ...
  constraint ai_owner_stepup_status_check check (
    status in ('verified', 'consumed', 'revoked', 'expired')
  ),
  constraint ai_owner_stepup_expiry_check check (expires_at > verified_at),
  constraint ai_owner_stepup_consumed_time_check check (
    (status <> 'consumed') or consumed_at is not null
  ),
  constraint ai_owner_stepup_revoked_time_check check (
...
begin
  ...
  if not (v_authorization.expires_at >= p_now) then
    update public.ai_owner_stepup_authorizations set status = 'expired', updated_at = p_now where id = p_authorization_id and status = 'verified';
    return query select false, 'STEPUP_EXPIRED'::text, v_authorization.id;
    return;
  end if;

  update public.ai_owner_stepup_authorizations set status = 'consumed', consumed_at = p_now, updated_at = p_now where id = p_authorization_id and status = 'verified' and owner_subject = p_owner_subject and action = p_action and release_digest = p_release_digest and payload_digest = p_payload_digest and scope_digest = p_scope_digest and environment = p_environment and expires_at >= p_now;
...
revoke all on function public.consume_ai_owner_stepup_authorization(uuid, text, text, text, text, text, text, timestamptz) from public, anon, authenticated;
grant execute on function public.consume_ai_owner_stepup_authorization(uuid, text, text, text, text, text, text, timestamptz) to service_role;
