...
create table if not exists tsn26_finance.settlements (
  ...
  constraint one_seller_or_direct_purchase check (
    (
      purchase_mode = 'DIRECT_SOVEREIGN_PURCHASE' and
      winning_claim_id is null and winning_role is null and
      discount_tmu * 10000 = list_price_tmu * 700 and
      general_manager_tmu = 0 and sector_manager_tmu = 0 and marketer_tmu = 0 and
      sales_absence_tmu * 10000 = collected_tmu * 2100 and
      rounding_reserve_tmu = 0
    )
    or
    (
      purchase_mode = 'ATTRIBUTED' and
      winning_claim_id is not null and winning_role is not null and discount_tmu = 0 and
      sales_absence_tmu * 10000 = collected_tmu * 1400 and
      rounding_reserve_tmu = 0 and
      (
...
create table if not exists tsn26_finance.settlement_state_events (
  ...
  state text not null check (state in (
    'PENDING', 'VALIDATED', 'VESTED', 'PAYABLE', 'SCHEDULED', 'PAID',
    'HELD', 'COMPLIANCE_REVIEW', 'REVERSED', 'CLAWED_BACK'
  )),
...
before update or delete on tsn26_finance.payment_events
for each row execute function tsn26_finance.deny_financial_mutation();

create trigger trg_sale_claims_append_only
before update or delete on tsn26_finance.sale_claims
for each row execute function tsn26_finance.deny_financial_mutation();

create trigger trg_settlements_append_only
before update or delete on tsn26_finance.settlements
for each row execute function tsn26_finance.deny_financial_mutation();

create trigger trg_settlement_state_events_append_only
before update or delete on tsn26_finance.settlement_state_events
for each row execute function tsn26_finance.deny_financial_mutation();
