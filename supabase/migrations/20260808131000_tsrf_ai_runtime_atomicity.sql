-- TSRF semantic convergence: atomic AI runtime persistence.
-- Depends on 20260808130000_tsrf_ai_trust_fabric.sql.
...
create table if not exists public.ai_runtime_reservations (
  ...
  constraint ai_runtime_reservation_release_digest_check check (release_digest ~ '^[0-9a-f]{64}$'),
  ...
  constraint ai_runtime_reservation_idempotency_check check (char_length(idempotency_key) between 8 and 256),
...
create index if not exists ai_runtime_reservations_release_idx
  on public.ai_runtime_reservations (release_digest, created_at desc);
...
create table if not exists public.ai_audit_chain_events (
  ...
  constraint ai_audit_chain_release_digest_check check (release_digest ~ '^[0-9a-f]{64}$'),
...
alter table public.ai_runtime_reservations force row level security;
alter table public.ai_runtime_daily_counters force row level security;
alter table public.ai_runtime_minute_counters force row level security;
alter table public.ai_runtime_concurrency_counters force row level security;
alter table public.ai_audit_chain_heads force row level security;
alter table public.ai_audit_chain_events force row level security;
...
begin
  ...
  insert into public.ai_runtime_reservations (
    ...
    p_estimated_cost_microusd, 'reserved', v_day, v_minute_bucket, p_now,
...
begin
  ...
  insert into public.ai_audit_chain_events (
    ...
    p_correlation_id, p_actor_subject, p_agent_id, p_decision, p_reason_code, p_metadata, p_now
  );

  update public.ai_audit_chain_heads
    set head_hash = p_event_hash, sequence_no = v_next_sequence, updated_at = p_now
    where stream_key = p_stream_key;
...
revoke all on function public.reserve_ai_runtime_capacity(text, text, text, text, text, bigint, timestamptz, integer) from public, anon, authenticated;
grant execute on function public.reserve_ai_runtime_capacity(text, text, text, text, text, bigint, timestamptz, integer) to service_role;
...
revoke all on function public.append_ai_audit_chain_event(text, text, text, text, text, text, text, text, text, jsonb, timestamptz) from public, anon, authenticated;
grant execute on function public.append_ai_audit_chain_event(text, text, text, text, text, text, text, text, text, jsonb, timestamptz) to service_role;
