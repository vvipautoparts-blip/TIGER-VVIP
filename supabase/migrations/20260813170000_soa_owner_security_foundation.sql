-- TIGER VVIP SOA-01: Sovereign Owner Access security foundation.
-- Repository contract only. No owner PII, credential material, remote apply, or production activation.
...
create unique index if not exists soa_owner_single_live_authority_idx
on public.soa_owner_authority_bindings (authority_scope)
where authority_status in ('VERIFIED', 'ACTIVE', 'SUSPENDED');
...
create table if not exists public.soa_owner_public_profiles (
  ...
  verified_owner_badge boolean not null default false,
  approved_public_contact_url text,
  publication_status text not null default 'DRAFT',
  public_version bigint not null default 1,
...
create table if not exists public.soa_owner_private_vault (
  ...
  classification text not null default 'OWNER_RESTRICTED',
...
create table if not exists public.soa_owner_authorization_leases (
  ...
  constraint soa_owner_lease_window_check check (
...
create table if not exists public.soa_owner_security_state (
  ...
  security_hold_state text not null default 'ACTIVE',
...
create policy soa_owner_public_profile_published_read
on public.soa_owner_public_profiles
for select
...
create trigger soa_owner_audit_append_only_guard
before update or delete on public.soa_owner_audit_events
for each row execute function public.soa_reject_owner_audit_mutation();

comment on table public.soa_owner_authority_bindings is 'SOA server-owned sovereign owner authority. Legacy browser roles are not authority.';
comment on table public.soa_owner_public_profiles is 'Explicitly published owner projection only; no private owner fields.';
comment on table public.soa_owner_private_vault is 'Encrypted owner envelope only; encryption keys remain outside this table and outside browser code.';
comment on table public.soa_owner_audit_events is 'Append-only owner security audit metadata. Never store credential or vault plaintext.';
comment on table public.soa_owner_authorization_leases is 'Short-lived exact-bound L4 lease; server/database-owned issuance time; one nonce, one consumption.';
comment on table public.soa_owner_security_state is 'Fail-closed owner security state. Initial kill switch is ON and L4 is disabled.';
